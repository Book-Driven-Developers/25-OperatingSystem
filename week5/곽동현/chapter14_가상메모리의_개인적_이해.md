...
프로세스는 거대한 가상 주소 공간을 부여받지만, **실제 물리 메모리는 접근한 페이지에 대해서만 할당**된다.
...
**나의 RAM 메모리공간보다 프로그램 크기가 훨씬 큰데, 어떻게 게임이 원활하게 실행될 수 있는걸까?**

### 답의 핵심

게임은 처음부터 전체 데이터를 메모리에 올릴 생각이 없다.   
가상 메모리 시스템을 활용하여 필요한 부분만 선택적으로 로드한다. (페이지, 프레임 기법의 핵심)

---

## EX 1) 오픈월드 게임 실행 과정 에서 발생하는 메모리 할당 과정 분석
...
-> 이 시점에는 물리 메모리가 거의 사용되지 않는다. 단지 "설계도"만 만들어진 상태다. (가상 메모리 준비)

### 2단계: 게임 시작 위치 - 최초 로딩
게임을 로딩하면, 특정 세이브 파일에 존재하는 나의 이전 마지막 내역들이 존재할 것이다. (로컬 혹은 게임사 서버에)
...
**사용자 체감**: 최초 긴 로딩 화면 (게임 시작 로딩이 가장 긴 이유)
...
```
논리적으로 존재하지만(프로그램에는 존재) 물리 메모리(RAM)에 올라오지 않은 정보:
- 5km 떨어진 던전
- 아직 발견하지 못한 지역
- 다른 대륙
- 엔딩 씬 데이터
...
이 과정에서 사용자는 연속된 흐름처럼 느껴지게된다. (실제로는 다양한 스왑인/아웃, 물리주소 찾는 과정이 들어가지만)
...
## EX 2) 메이플스토리 포탈 이동과 메모리 할당 과정 분석

### 온라인 게임의 구조적 특징

#### 클라이언트(내 컴퓨터)에 저장된 것 (거의 모든 정보)
```
설치 디렉토리 구조:
├── maps/
...
총 용량: 약 15GB
특징 : 고정되어 있는 정보(맵정보, 몬스터 체력, NPC정보 등)는 나의 프로그램이 가지고 있음 
```

#### 서버에서 관리하는 것
```
메모리 데이터 ("실시간" 숫자와 상태만 관리):
- 맵 ID: 100000001 (헤네시스)
...
특징: 매우 작음 (KB 단위) 하지만 모두 실시간으로 변동되고 공유됨
```
