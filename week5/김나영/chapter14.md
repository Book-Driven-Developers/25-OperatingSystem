...
## 개요
페이지 테이블의 유효 비트(Valid Bit) 개념을 Java로 구현한 시뮬레이션입니다. 프로그램 시작 시 모든 페이지의 유효 비트는 0으로 초기화됩니다(메모리에 없음을 의미). CPU가 유효 비트가 0인 페이지에 접근하면 페이지 폴트가 발생하고, OS가 해당 페이지를 메모리에 로드한 후 유효 비트를 1로 변경합니다. 같은 페이지를 다시 접근하면 유효 비트가 1이므로 페이지 폴트 없이 정상 접근됩니다.

## 실험 결과
총 5회 페이지 접근 중 첫 접근 시에만 2회 페이지 폴트 발생, 나머지 3회 재접근은 모두 정상 접근되었습니다. 이는 실제 OS의 Demand Paging(요구 페이징) 전략과 동일한 동작으로, 필요한 페이지만 메모리에 로드하여 메모리를 효율적으로 사용합니다.

```
=== 페이지 폴트 시뮬레이션 ===

유효 비트 = 0 : 페이지가 메모리에 없음 → 페이지 폴트!
유효 비트 = 1 : 페이지가 메모리에 있음 → 정상 접근
...
[ 분석 ]
- 페이지 1, 2 첫 접근: 페이지 폴트 발생 (유효 비트 = 0)
- 페이지 1, 2 재접근: 정상 접근 (유효 비트 = 1)
```